...
class LearningExperience:
    ...
    def add_deep_experience(self, reward, state, done):
        if self.previous_state is not None:
            experience = (self.previous_state, reward, state, done)
    ...
    def count_moves(self, data=None):
        ...
        for experience in data:
            _, _, state, _ = experience
            move_tensor = state[5]
            move_direction = move_tensor.index(max(move_tensor))
